<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="报名ID" prop="enrollmentId" width="120" />
        <el-table-column align="left" label="操作类型（增加或扣除）" prop="action" width="120" />
        <el-table-column align="left" label="操作原因" prop="reason" width="120" />
        <el-table-column align="left" label="课时数量" prop="numSessions" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateEduClassSessionFunc(scope.row)">变更</el-button>
...
const onDelete = async() => {
      ...
      const res = await deleteEduClassSessionByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateEduClassSessionFunc = async(row) => {
    const res = await findEduClassSession({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
